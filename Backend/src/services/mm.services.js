...
async function searchIOR(temp) {
    ...
    if(numberRegex.test(input)){
        ...
        OR subject_ior::TEXT ILIKE '%${input}%' OR occur_nbr ILIKE '%${input}%' OR reference_ior ILIKE '%${input}%'`;
    }
    console.log(query);
    const result = await db.query(query);
    if (result.rowCount) {
...
async function addFollowUpOccurrence(followUpData) {
    ...
    console.log("Follow Up IOR\n", followUpData);
    // Fetch documentid
    const query = `SELECT documentid FROM tbl_occurrence WHERE id_ior = ${id_IOR}`;
    const result = await db.query(query);
    const documentid = result.rows[0].documentid;
    const ReferenceReplacement = {
        '{Ref1Yes}': follup_datarefer === 'yes' ? '☑ Yes' : '☐ Yes',
        '{Ref1No}': follup_datarefer === 'no' ? '☑ No' : '☐ No'
    };

    const StatusReplacement = {
        '{Open1}': follup_status === 'open' ? '☑ Open' : '☐ Open',
        '{Close1}': follup_status === 'close' ? '☑ Close' : '☐ Close'
    };

    // Replace placeholders in the document
    const replacements = {
        '{FollowOn1}': follup_detail,
        '{NameFollow1}': follupby,
        '{UnitFollow1}': follup_uic,
        '{DateFollow1}': follup_date,
        '{NextUnit}': nextuic_follup,
        '{Curr_Prob}': current_probability,
        '{Curr_Severity}': current_severity,
        '{Curr_Risk}': current_riskindex,
        ...ReferenceReplacement,
        ...StatusReplacement,
    };
    console.log(replacements);
    for (const [placeholder, replacement] of Object.entries(replacements)) {
        await replaceTextInGoogleDocs(documentid, placeholder, replacement);
    }
    query = `INSERT INTO tbl_follupOccur (
        id_IOR,
        follup_detail,
        follupby,
        follup_uic,
...
async function searchNCR(temp) {
    ...
    if(numberRegex.test(input)){
        ...
        OR subject ILIKE '%${input}%' OR regulationbased::TEXT ILIKE '%${input}%'`;
    }
    console.log(query);
    const result = await db.query(query);
    if (result.rowCount) {
...
async function addNCRReply(mm) {
    const { accountid, ncr_init_id, root_cause, identified_date, accepted_by, accepted_date } = mm;
    try {
        // Fetch documentid
        let query = `SELECT documentid FROM NCR_Initial WHERE ncr_init_id = ${ncr_init_id}`;
        ...
        console.log(query);
        result = await db.query(query);
        const identified_by = result.rows[0].name;
        console.log("Identified by: ", identified_by);
        ...
        const replacements = {
            ...
            '{Accept_by}': accepted_by,
            '{Accept_date}': accepted_date,
        };

        for (const [placeholder, replacement] of Object.entries(replacements)) {
            await replaceTextInGoogleDocs(documentid, placeholder, replacement);
        }
        console.log("NCR Replied");

        // Insert into NCR_reply
        query = `
            INSERT INTO NCR_reply (
                AccountID, 
                NCR_init_ID, 
        ...
        if (result.rowCount === 1) {
            ...
            throw new Error('Error inserting NCR reply');
        }
    } catch (error) {
...
async function addNCRFollowResult(temp) {
    ...
    console.log(temp);
    const query = `SELECT documentid FROM NCR_Initial WHERE ncr_init_id = ${ncr_init_id}`
    const documentid = (await db.query(query)).rows[0].documentid;
    console.log("DocumentID: ", documentid);
    console.log("Accound id: ", accountid);
    const replacements = {
        '{Root_Cause}' : audit_no,
        '{NCR_No}': ncr_no,
        '{IssuedDate}': issued_date,
